 ...
 * @author Jezza
 */
public final class ProtoInterpreter {
...
//	}
	...
	static class CS extends MutableCallSite {
		CS() {
			super(methodType(void.class, ProtoInterpreter.class, StackFrame.class, byte.class));
			setTarget(FALLBACK.bindTo(this));
		}

		private void fallback(ProtoInterpreter interpreter, StackFrame frame, byte op) throws Throwable {
		...
		private static boolean test(ProtoInterpreter interpreter, StackFrame frame, byte op, byte expected) {
			return op == expected;
		}
		...
		static {
			...
			try {
				FALLBACK = lookup.findVirtual(CS.class, "fallback", methodType(void.class, ProtoInterpreter.class, StackFrame.class, byte.class));
				TEST = lookup.findStatic(CS.class, "test", methodType(boolean.class, ProtoInterpreter.class, StackFrame.class, byte.class, byte.class));
...
//				TEST = MethodHandles.dropArguments(test, 0, Interpreter.class, StackFrame.class);
			...
			for (Field field : fields) {
				if (field.isAnnotationPresent(Implemented.class)) {
					try {
						handles.add(lookup.in(ProtoInterpreter.class)
								.findVirtual(ProtoInterpreter.class, field.getName(), methodType(void.class, StackFrame.class)));
					} catch (NoSuchMethodException | IllegalAccessException e) {
						throw new AssertionError(e);
					}
				} else {
	...
	public ProtoInterpreter() {
		this(new StackFrame());
	}

	public ProtoInterpreter(StackFrame initialFrame) {
		status = 0;
		stack = new Object[0];
		frameIndex = 1;
...
//		dispatchNext(MOV_MH, frame);
	...
	private static int print(ProtoInterpreter interpreter, StackFrame frame) {
	...
	private static int nativeAdd(ProtoInterpreter interpreter, StackFrame frame) {
		int parameters = frame.top - frame.base;
		if (parameters != 2)
	...
	private static int toLower(ProtoInterpreter interpreter, StackFrame frame) {
		int parameters = frame.top - frame.base;
		if (parameters != 1)
	...
	private static int toUpper(ProtoInterpreter interpreter, StackFrame frame) {
		int parameters = frame.top - frame.base;
		if (parameters != 1)
			throw new IllegalStateException("Requires 1 argument.");
		String o = (String) interpreter.stackPop(frame);
	...
	private static void prep(ProtoInterpreter interpreter) {
		interpreter.globals.put("print", (Callback) ProtoInterpreter::print);
		interpreter.globals.put("native_add", (Callback) ProtoInterpreter::nativeAdd);
		interpreter.globals.put("upper", (Callback) ProtoInterpreter::toUpper);
		interpreter.globals.put("lower", (Callback) ProtoInterpreter::toLower);
	}
	...
	public static void test(LuaChunk chunk) throws Throwable {
		...
		ProtoInterpreter interpreter = new ProtoInterpreter(frame);
		prep(interpreter);
		interpreter.execute();
	}
...
//		private int tailcalls;
	...
	interface Callback {
		int call(ProtoInterpreter interpreter, StackFrame frame);
	}
